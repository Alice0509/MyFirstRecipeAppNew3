  ...
  const renderNode = (node, index) => {
    ...
    switch (node.nodeType) {
      ...
      case INLINES.HYPERLINK:
        return (
          <TouchableOpacity key={index} onPress={() => Linking.openURL(node.data.uri)}>
      ...
      case MARKS.BOLD:
        return <Text key={index} style={styles.bold}>{node.value}</Text>;
      case MARKS.ITALIC:
        return <Text key={index} style={styles.italic}>{node.value}</Text>;
      default:
